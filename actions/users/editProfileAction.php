...
if(isset($_POST['valid'])){
    ...
    if(isset($_FILES['newpp']) AND !empty($_FILES['newpp']['name'])){
        $maxsize = 512000; // 2097152 = 2 megabytes
            ...
            if(in_array($uploadedExtension, $supportedExtensions)){
                ...
                if($result){
                    ...
                    if(isset($_POST['newpseudo']) AND !empty($_POST['newpseudo'])){
                        ...
                        $editPost = $db->prepare('UPDATE users SET pseudo = ? WHERE id = ?');
                        $editPost->execute(array($new_pseudo, $_GET['id']));
                    ...
                    if(isset($_POST['newmail']) AND !empty($_POST['newmail'])){
                        ...
                        $editPost = $db->prepare('UPDATE users SET email = ? WHERE id = ?');
                        $editPost->execute(array($new_mail, $_GET['id']));
                    ...
                    if(!empty($_POST['newpw']) AND $_POST['newpw'] == $_POST['newpwConfirm']){
                        ...
                        $editPost = $db->prepare('UPDATE users SET password = ? WHERE id = ?');
                        $editPost->execute(array($new_password, $_GET['id']));
